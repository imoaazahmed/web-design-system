...
export interface UseCarouselProps {
  ...
  isDisabled?: boolean;
  /**
   * If `true`, swiping the carousel will be disabled
   */
...
export function useCarousel(props: UseCarouselProps = {}) {
  ...
  const themeIsRTL = useIsRTL();
  const isRtl = langDirection === 'rtl' || themeIsRTL;
  /**
  ...
  const hasFakeSlides = useMemo(() => loop && perView === 1 && pages > 1, [
    loop,
    perView,
    pages,
  ]);
  ...
  const handleSwiping = (e: EventData) => {
    ...
    if (e.dir === 'Left') {
      setTempTranslateX(isRtl ? translateX - e.absX : translateX + e.absX);
    }
    if (e.dir === 'Right') {
      setTempTranslateX(isRtl ? translateX + e.absX : translateX - e.absX);
    }
  ...
  const swipeableProps = useSwipeable({
    ...
    preventDefaultTouchmoveEvent: (_, direction) => {
      if (
  ...
  return {
    ...
    getSlideGroupWrapperProps: (props: Dict = {}) => {
      return {
        ...props,
        overflow: 'hidden',
        paddingStart: offsetBeforeProp + 'px',
        paddingEnd: offsetAfterProp + 'px',
      };
    },
